...
public class BridleLayer extends LayerRenderer<SWEMHorseEntity, SWEMHorseModel> {

	private final SWEMHorseModel model = new SWEMHorseModel();

	public BridleLayer(IEntityRenderer<SWEMHorseEntity, SWEMHorseModel> entityRendererIn) {
	...
	public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, SWEMHorseEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		ItemStack stack = entitylivingbaseIn.getBridle();
		if (!stack.isEmpty()) {
			BridleItem bridle = (BridleItem)stack.getItem();
			...
			IVertexBuilder ivertexbuilder = bufferIn.getBuffer(RenderType.getEntityCutoutNoCull(bridle.getArmorTexture()));
