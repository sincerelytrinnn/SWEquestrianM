...
public class SWEMHorseSaddle extends NamedEntityFix {
	public SWEMHorseSaddle(Schema outputSchema, boolean changesType) {
	...
	protected Typed<?> fix(Typed<?> p_207419_1_) {
		...
		if (!optional.isPresent() && dynamic.get("Saddle").asBoolean(false)) {
			...
			typed = typed.set(opticfinder, Pair.of(TypeReferences.ITEM_NAME.typeName(), "swem:western_saddle_light_blue"));
