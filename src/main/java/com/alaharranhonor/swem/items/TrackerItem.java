		...
		if (!worldIn.isRemote) {
			...
			for (int i = 0; i < tracked.size(); i++) {
				...
				if (entity instanceof SWEMHorseEntityBase) {
					builder.append(entity.getDisplayName()).append(" x: ").append(entity.getPosition().getX()).append(" - y: ").append(entity.getPosition().getY()).append(" - z: ").append(entity.getPosition().getZ()).append("\n");
				}
			}
