...
public class BlockStates extends BlockStateProvider {
  ...
  protected void registerGrainFeeders() {
    ...
    for (RegistryObject<GrainFeederBlock> gf : SWEMBlocks.GRAIN_FEEDERS) {
      ...
      // itemModels().withExistingParent("item/" + feeder.getName().split("\\.")[2],
      // "item/generated")
      //		.texture("layer0", new ResourceLocation(SWEM.MOD_ID, "items/" +
      // feeder.getName().split("\\.")[2]));
      ...
      getVariantBuilder(feeder)
          .forAllStates(
              (state) -> {
                ...
                boolean corner =
                    state.getValue(GrainFeederBlock.LEFT) || state.getValue(GrainFeederBlock.RIGHT);
                ModelFile model =
                    models()
                        .getBuilder(
                            "grain_feeder_"
                                + (corner ? "corner_" : "")
                                + feeder.getColour().getName())
                        .texture(
                            "0",
                            new ResourceLocation(
                                SWEM.MOD_ID,
                                "blocks/grain_feeder_"
                                    + (corner ? "corner_" : "")
                                    + feeder.getColour().getName()))
                        .texture(
                            "particle",
  ...
  protected void registerNonParallelBlock(List<RegistryObject<NonParallelBlock>> blockArray) {
    ...
    for (RegistryObject<NonParallelBlock> sep : blockArray) {
      ...
      itemModels()
          .withExistingParent(
              "item/" + sepBlock.getName().getString().split("\\.")[2], "item/generated")
          .texture(
              "layer0",
              new ResourceLocation(
                  SWEM.MOD_ID, "items/" + sepBlock.getName().getString().split("\\.")[2]));
              ...
              (state) -> {
                ...
                ModelFile model =
                    models()
                        .getBuilder(
                            "separator_"
                                + state.getValue(NonParallelBlock.PART).getSerializedName()
                                + "_"
                                + sepBlock.getColour().getName())
                        .texture(
                            "0",
                            new ResourceLocation(
                                SWEM.MOD_ID,
                        ...
                        .parent(
                            models()
                                .getBuilder(models[state.getValue(NonParallelBlock.PART).getId()]));
  ...
  protected void registerWheelBarrows() {
    ...
    for (RegistryObject<WheelBarrowBlock> wb : SWEMBlocks.WHEEL_BARROWS) {
      ...
      itemModels()
          .withExistingParent(
              "item/" + wheel.getName().getString().split("\\.")[2], "item/generated")
          .texture(
              "layer0",
              new ResourceLocation(
                  SWEM.MOD_ID, "items/" + wheel.getName().getString().split("\\.")[2]));
          ...
          .forAllStates(
              (state) -> {
                ...
                ModelFile model =
                    models()
                        .getBuilder(
                            models[state.getValue(WheelBarrowBlock.LEVEL)]
                                + "_"
                                + wheel.getColour().getName())
                        .texture(
                            "0",
                            new ResourceLocation(
                                SWEM.MOD_ID, "blocks/wheel_barrow_" + wheel.getColour().getName()))
                        .texture(
                            "particle",
                            new ResourceLocation(
                                SWEM.MOD_ID, "blocks/wheel_barrow_" + wheel.getColour().getName()))
                        .parent(
                            models().getBuilder(models[state.getValue(WheelBarrowBlock.LEVEL)]));
                return ConfiguredModel.builder()
                    .modelFile(model)
                    .rotationY(
                        dir.getAxis() != Direction.Axis.Y
  ...
  private void registerSlowFeeders() {
    ...
    for (RegistryObject<SlowFeederBlock> sf : SWEMBlocks.SLOW_FEEDERS) {
      ...
      itemModels()
          .withExistingParent(
              "item/" + slow.getName().getString().split("\\.")[2], "item/generated")
          .texture(
              "layer0",
              new ResourceLocation(
                  SWEM.MOD_ID, "items/" + slow.getName().getString().split("\\.")[2]));

      ModelFile sfModel =
          models()
              .getBuilder(models[0] + "_" + slow.getColour().getName())
              .texture(
                  "0",
                  new ResourceLocation(
                      SWEM.MOD_ID,
                      "blocks/" + models[0].split("/")[1] + "_" + slow.getColour().getName()))
              .texture(
                  "particle",
                  new ResourceLocation(
                      SWEM.MOD_ID,
      ...
      ModelFile sfCornerModel =
          models()
              .getBuilder(models[1] + "_" + slow.getColour().getName())
              .texture(
                  "0",
                  new ResourceLocation(
                      SWEM.MOD_ID,
                      "blocks/" + models[1].split("/")[1] + "_" + slow.getColour().getName()))
              .texture(
                  "particle",
                  new ResourceLocation(
                      SWEM.MOD_ID,
      ...
      mp.part()
          .modelFile(
              models().getExistingFile(new ResourceLocation(SWEM.MOD_ID, "block/full_feeder")))
          .addModel()
          .condition(SlowFeederBlock.LEVEL, 2);
      mp.part()
          .modelFile(
              models().getExistingFile(new ResourceLocation(SWEM.MOD_ID, "block/half_feeder")))
          .addModel()
          .condition(SlowFeederBlock.LEVEL, 1);
