...
public class HorseDoorBlock extends Block {
  ...
  public boolean isPathfindable(
      BlockState pState, IBlockReader pLevel, BlockPos pPos, PathType pType) {
    switch (pType) {
      case LAND:
    ...
    BlockPos.betweenClosed(
            ...
            blockpos
                ...
                .relative(
                    ...
                    direction == Direction.EAST ? 1 : direction == Direction.NORTH ? 1 : -1))
        .forEach(
            blockPos1 -> {
          ...
          case NORTH:
            {
              worldIn.setBlock(
                  pos.west(),
                  state.setValue(SIDE, SWEMBlockStateProperties.DoubleBlockSide.LEFT),
                  3);
              worldIn.setBlock(
                  pos.west().above(),
                  state
            ...
            case EAST:
              {
                if (state.getValue(HINGE) == DoorHingeSide.LEFT) {
                  worldIn.setBlock(
                      pos.offset(1, 0, -1), state.setValue(OPEN, Boolean.valueOf(open)), 10);
                  break;
                } else {
                  worldIn.setBlock(
                      pos.offset(1, 0, 1), state.setValue(OPEN, Boolean.valueOf(open)), 10);
  ...
  public ArrayList<BlockPos> getAllDoorParts(
    ...
    if (state.getValue(HINGE) == DoorHingeSide.LEFT
        && state.getValue(SIDE) == SWEMBlockStateProperties.DoubleBlockSide.LEFT) {
      ...
      if (state.getValue(HALF) == DoubleBlockHalf.LOWER) {
        ...
        && state.getValue(SIDE) == SWEMBlockStateProperties.DoubleBlockSide.RIGHT) {
