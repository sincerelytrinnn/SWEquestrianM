...
public class WaterTroughBlock extends NonParallelBlock {
    ...
    if (itemstack.isEmpty()) {
      ...
      if (item == Items.WATER_BUCKET) {
        if (i < 16 && !worldIn.isClientSide) {
          ...
          worldIn.playSound(
              (PlayerEntity) null, pos, SoundEvents.BUCKET_EMPTY, SoundCategory.BLOCKS, 1.0F, 1.0F);
        }
        ...
        if (i >= 1 && !worldIn.isClientSide) {
          ...
          worldIn.playSound(
              (PlayerEntity) null, pos, SoundEvents.BUCKET_FILL, SoundCategory.BLOCKS, 1.0F, 1.0F);
        }
  ...
  public boolean canSurvive(BlockState state, IWorldReader world, BlockPos pos) {
    ...
    if ((checkLeft.getBlock() instanceof WaterTroughBlock
