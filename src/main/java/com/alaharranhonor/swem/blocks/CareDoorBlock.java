	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		...
		if (hinge == DoorHingeSide.LEFT) {
			...
			if (blockpos.getY() < 254 && blockChecks.stream().allMatch((bool) -> bool)) {
				...
				return this.defaultBlockState().setValue(FACING, context.getHorizontalDirection()).setValue(HINGE, this.getHingeSide(context)).setValue(OPEN, Boolean.valueOf(flag)).setValue(HALF, DoubleBlockHalf.LOWER).setValue(SIDE, SWEMBlockStateProperties.TripleBlockSide.LEFT);
			} else {
				return null;
			}
		} else {
			...
			if (blockpos.getY() < 254 && blockChecks.stream().allMatch((bool) -> bool)) {
				...
				boolean flag = world.hasNeighborSignal(blockpos) || world.hasNeighborSignal(blockpos.above());
				return this.defaultBlockState().setValue(FACING, context.getHorizontalDirection()).setValue(HINGE, this.getHingeSide(context)).setValue(OPEN, Boolean.valueOf(flag)).setValue(HALF, DoubleBlockHalf.LOWER).setValue(SIDE, SWEMBlockStateProperties.TripleBlockSide.RIGHT);
			} else {
				return null;
			}
		}
