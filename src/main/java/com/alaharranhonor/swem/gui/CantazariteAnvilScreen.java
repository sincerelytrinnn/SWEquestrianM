...
public class CantazariteAnvilScreen extends AbstractRepairScreen<CantazariteAnvilContainer> {
    private static final ResourceLocation ANVIL_RESOURCE =
            new ResourceLocation(SWEM.MOD_ID, "textures/gui/container/cantazarite_anvil.png");
    private static final ITextComponent TOO_EXPENSIVE_TEXT =
            new TranslationTextComponent("swem.container.anvil");
     ...
     */
    public CantazariteAnvilScreen(
            CantazariteAnvilContainer container, PlayerInventory playerInventory, ITextComponent title) {
    ...
    protected void subInit() {
        ...
        this.nameField =
                new TextFieldWidget(
                        this.font, i + 62, j + 24, 103, 12, new TranslationTextComponent("container.repair"));
    ...
    private void renameItem(String name) {
        if (!name.isEmpty()) {
            ...
            if (slot != null
                    && slot.hasItem()
                    && !slot.getItem().hasCustomHoverName()
     ...
     * match the actual contents of that slot.
     */
