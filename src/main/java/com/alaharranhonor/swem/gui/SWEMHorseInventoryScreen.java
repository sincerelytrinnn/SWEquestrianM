...
@OnlyIn(Dist.CLIENT)
public class SWEMHorseInventoryScreen extends ContainerScreen<SWEMHorseInventoryContainer>
    implements IHasContainer<SWEMHorseInventoryContainer> {

  private static final ResourceLocation HORSE_GUI_TEXTURES =
      new ResourceLocation(SWEM.MOD_ID, "textures/gui/container/swem_horse.png");
   ...
   */
  public SWEMHorseInventoryScreen(
      SWEMHorseInventoryContainer p_i51084_1_,
  ...
  protected void init() {
    ...
    this.permissionButton =
        new CustomHeightButton(
            this.leftPos + 122,
            this.topPos + 111,
            48,
            14,
            new StringTextComponent(horseEntity.getPermissionState().name()),
            p_onPress_1_ -> {
              SWEMPacketHandler.INSTANCE.sendToServer(new HorseStateChange(9, horseEntity.getId()));
              p_onPress_1_.setMessage(
                  new StringTextComponent(
                      SWEMHorseEntityBase.RidingPermission.values()[
                          (horseEntity.getPermissionState().ordinal() + 1) % 3]
                          .name()));
            });

    if (!Objects.equals(
        this.horseEntity.getOwnerUUID(), Minecraft.getInstance().player.getUUID())) {
  ...
  protected void renderBg(MatrixStack matrixStack, float partialTicks, int x, int y) {
    ...
    if (tackSet != 0) {
      for (int xSlot = 0; xSlot < 2; xSlot++) {
        for (int ySlot = 0; ySlot < 3; ySlot++) {
          blit(
              matrixStack,
              this.leftPos + 8 + (xSlot * 21),
              this.topPos + 17 + (ySlot * 21),
              180 + (xSlot * 21),
              4 + (ySlot * 21) + (tackSet == 2 ? 67 : 0),
              16,
              16);
        }
      }
    ...
    // Render the horse
    // InventoryScreen.drawEntityOnScreen(i + 51, j + 60, 17, (float)(i + 51) - this.mousePosx,
  ...
  protected void renderLabels(MatrixStack matrixStack, int x, int y) {
    ...
    this.font.draw(
        matrixStack,
        new StringTextComponent(SWEMUtil.checkTextOverflow(this.horseEntity.getOwnerName(), 22)),
        65.2f,
        36.0f,
        4210752);
    ...
    if (jumpLeveling.getLevel() != jumpLeveling.getMaxLevel()) {
      jumpInfo =
          new TranslationTextComponent(
              String.format(
                  "%s: %.0f/%.0f",
                  jumpLeveling.getLevelName(), jumpLeveling.getXp(), jumpLeveling.getRequiredXp()));
    } else {
    ...
    this.font.draw(
        ...
        new TranslationTextComponent(
            String.format(
                "%s: %.1f/%.0f",
                healthLeveling.getLevelName(),
                this.horseEntity.getHealth(),
                this.horseEntity.getMaxHealth())),
        65.0f,
        78.0f,
        4210752);
