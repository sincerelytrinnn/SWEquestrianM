...
public class TackBoxDefaultScreen extends ContainerScreen<TackBoxContainer> {
  ...
  private static final ResourceLocation TACKBOX_DEFAULT_TEXTURE =
      new ResourceLocation(SWEM.MOD_ID, "textures/gui/container/tackbox_default.png");
   ...
   */
  public TackBoxDefaultScreen(
      TackBoxContainer screenContainer, PlayerInventory inv, ITextComponent titleIn) {
    super(
        screenContainer,
        inv,
        new StringTextComponent(
            screenContainer.horse.getDisplayName().getString() + "'s Tack Box"));
    this.imageWidth = 176;
  ...
  protected void init() {
    ...
    this.permissionButton =
        new CustomHeightButton(
            ...
            48,
            14,
            new StringTextComponent(getMenu().horse.getPermissionState().name()),
            p_onPress_1_ -> {
              SWEMPacketHandler.INSTANCE.sendToServer(
                  new HorseStateChange(9, getMenu().horse.getId()));
              p_onPress_1_.setMessage(
                  new StringTextComponent(
                      SWEMHorseEntityBase.RidingPermission.values()[
                          (getMenu().horse.getPermissionState().ordinal() + 1) % 3]
                          .name()));
            });
  ...
  protected void renderLabels(MatrixStack matrixStack, int x, int y) {
    ...
    this.font.draw(
        matrixStack, new StringTextComponent("Storage"), storageLabelX, storageLabelY, 4210752);
  }
