...
public class LookForWaterGoal extends Goal {
    ...
    public boolean canUse() {
        return !this.horse.isBaby()
            && this.horse.getNeeds().getThirst().getState().getId() < 3
            && this.horse.getPassengers().isEmpty()
            && !this.horse.getNeeds().getThirst().isOnCooldown()
    ...
    public void tick() {
        ...
        if (foundWater == null) {
            ...
            this.foundWater =
                !waterTroughs.isEmpty()
                    ? waterTroughs.get(this.horse.getRandom().nextInt(waterTroughs.size()))
                    : !halfBarrels.isEmpty()
                    ? halfBarrels.get(this.horse.getRandom().nextInt(halfBarrels.size()))
                    : !waterPools.isEmpty()
                    ? waterPools.get(this.horse.getRandom().nextInt(waterPools.size()))
                    : null;
            ...
            if (this.horse.blockPosition().closerThan(this.foundWater, 2)) {
                ...
                if (foundState.getBlock() instanceof WaterTroughBlock) {
                    ((WaterTroughBlock) foundState.getBlock())
                        .setWaterLevel(this.horse.level, this.foundWater, foundState, true);
                    this.horse.getNeeds().getThirst().incrementState();
                } else if (foundState.getBlock() instanceof HalfBarrelBlock) {
                    ((HalfBarrelBlock) foundState.getBlock())
                        .setWaterLevel(
                            this.horse.level,
                            this.foundWater,
                            foundState,
                            foundState.getValue(HalfBarrelBlock.LEVEL) - 1);
                    this.horse.getNeeds().getThirst().incrementState();
                ...
                if (this.movingTimer >= 200) {
                    ...
                    this.horse
                        .getNavigation()
                        .moveTo(foundWater.getX(), foundWater.getY(), foundWater.getZ(), this.speed);
                }
            }
        }
    }
