...
public class PoopGoal extends Goal {
    ...
    public boolean canUse() {
        return !this.pooperEntity.isBaby()
                && this.pooperEntity.level.getGameTime()
                % (ConfigHolder.SERVER.serverPoopInterval.get() * 20 + pooperEntity.getId())
                == 0
                && this.pooperEntity.getPassengers().isEmpty()
                && ConfigHolder.SERVER.serverTickPoopNeed.get();
    }
    ...
    public void tick() {
        ...
        if (this.poopTimer == 48) {
            ...
            BlockPos posToPoop =
                    blockpos.offset(0, 1.5d, 0).relative(this.pooperEntity.getDirection().getOpposite());
            if (poop != null) {
