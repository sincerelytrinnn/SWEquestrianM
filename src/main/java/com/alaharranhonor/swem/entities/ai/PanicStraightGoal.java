...
public class PanicStraightGoal extends Goal {
    ...
    protected boolean findRandomPosition() {
        ...
        vector3i =
                vector3i.relative(
                        direction,
                        MathHelper.clamp(
                                this.horse.progressionManager.getAffinityLeveling().getMaxLevel()
                                        - this.horse.progressionManager.getAffinityLeveling().getLevel(),
                                4,
                                14));
    ...
    public boolean canUse() {
        if (this.horse.getLastHurtByMob() == null && !this.horse.isOnFire()
                || this.horse.isVehicle()
                || this.horse.isBaby()) {
            return false;
        } else {
    ...
    protected BlockPos lookForWater(
            IBlockReader pLevel, Entity pEntity, int pHorizontalRange, int pVerticalRange) {
