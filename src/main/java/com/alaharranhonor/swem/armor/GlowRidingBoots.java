...
public class GlowRidingBoots extends LeatherRidingBoots {
  ...
  public void appendHoverText(
    ...
    p_77624_3_.add(
        new StringTextComponent("Adds Light-Jump")
    ...
    public static void onJump(LivingEvent.LivingJumpEvent event) {
      ...
      if (stack.getItem() instanceof GlowRidingBoots) {
        ...
        if (!stackData.contains("glowBlockPos")) {
          ...
          world.setBlock(
              glowBlockPos, SWEMBlocks.INVISIBLE_GLOW_BLOCK.get().defaultBlockState(), 3);
        } else {
          CompoundNBT glowBlockData = (CompoundNBT) stackData.get("glowBlockPos");
          BlockPos oldGlowBlockPos =
              new BlockPos(
                  glowBlockData.getInt("x"), glowBlockData.getInt("y"), glowBlockData.getInt("z"));
          if (!glowBlockPos.equals(oldGlowBlockPos)) {
            if (world.getBlockState(oldGlowBlockPos).getBlock()
                == SWEMBlocks.INVISIBLE_GLOW_BLOCK.get()) {
      ...
      if ((event.getFrom().getItem() instanceof GlowRidingBoots)) {
        ...
        if (stackData != null) {
          if (stackData.contains("glowBlockPos")) {
            ...
            BlockPos oldGlowBlockPos =
                new BlockPos(
                    glowBlockPos.getInt("x"), glowBlockPos.getInt("y"), glowBlockPos.getInt("z"));
            event
                .getEntityLiving()
                .getCommandSenderWorld()
                .setBlock(oldGlowBlockPos, Blocks.AIR.defaultBlockState(), 3);
          }
        }
      }
    }
