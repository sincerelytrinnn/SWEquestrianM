package com.alaharranhonor.swem.world.structure;
...
public class CottageStructure extends Structure<NoFeatureConfig> {
    ...
    /**
     * This is how the worldgen code knows what to call when it is time to create the pieces of the
    ...
    /**
     * : WARNING!!! DO NOT FORGET THIS METHOD!!!! : If you do not override step method, your structure
     * WILL crash the biome as it is being parsed!
     *
     * <p>Generation stage for when to generate the structure. there are 10 stages you can pick from!
     ...
     *
     * <p>This method allows us to have mobs that spawn naturally over time in our structure. No other
     * mobs will spawn in the structure of the same entity classification. The reason you want to
     * match the classifications is so that your structure's mob will contribute to that
     * classification's cap. Otherwise, it may cause a runaway spawning of the mob that will never
     * stop.
     *
     * <p>NOTE: getDefaultSpawnList is for monsters only and getDefaultCreatureSpawnList is for
     * creatures only. If you want to add entities of another classification, use the
     * StructureSpawnListGatherEvent to add water_creatures, water_ambient, ambient, or misc mobs. Use
     * that event to add/remove mobs from structures that are not your own.
     *
     * <p>NOTE 2: getSpecialEnemies and getSpecialAnimals are the vanilla methods that Forge does not
     * hook up. Do not use those methods or else the mobs won't spawn. You would have to manually
     * implement spawning for them. Stick with Forge's Default form as it is easier to use that.
     */
  /*
  ...
  public List<MobSpawnInfo.Spawners> getDefaultCreatureSpawnList() {
    ...
    protected boolean isFeatureChunk(
        ...
        int landHeight =
            chunkGenerator.getFirstOccupiedHeight(
                centerOfChunk.getX(), centerOfChunk.getZ(), Heightmap.Type.WORLD_SURFACE_WG);

        // Grabs column of blocks at given position. In overworld, this column will be made of stone,
        // water, and air.
        // In nether, it will be netherrack, lava, and air. End will only be endstone and air. It
        // depends on what block
        // the chunk generator will place for that dimension.
        IBlockReader columnOfBlocks =
            chunkGenerator.getBaseColumn(centerOfChunk.getX(), centerOfChunk.getZ());

        // Combine the column of blocks with land height and you get the top block itself which you can
        // test.
        ...
        BlockState topBlockThreeDown =
            columnOfBlocks.getBlockState(centerOfChunk.above(landHeight - 3));
        ...
        return topBlock.getFluidState().isEmpty()
            && topBlockOneDown.getFluidState().isEmpty()
            && topBlockTwoDown.getFluidState().isEmpty()
            && topBlockThreeDown.getFluidState().isEmpty()
            && topBlockFourDown.getFluidState().isEmpty()
            && !isNearVillage(chunkGenerator, seed, chunkRandom, chunkX, chunkZ); // landHeight > 100;
    }
    ...
    private boolean isNearVillage(
        ChunkGenerator p_242782_1_,
        long p_242782_2_,
        SharedSeedRandom p_242782_4_,
        int p_242782_5_,
        int p_242782_6_) {
        StructureSeparationSettings structureseparationsettings =
            p_242782_1_.getSettings().getConfig(Structure.VILLAGE);
        if (structureseparationsettings == null) {
            ...
            for (int i = p_242782_5_ - 10; i <= p_242782_5_ + 10; ++i) {
                for (int j = p_242782_6_ - 10; j <= p_242782_6_ + 10; ++j) {
                    ChunkPos chunkpos =
                        Structure.VILLAGE.getPotentialFeatureChunk(
                            structureseparationsettings, p_242782_2_, p_242782_4_, i, j);
        ...
        public void generatePieces(
            ...
            JigsawManager.addPieces(
                ...
                new VillageConfig(
                    ...
                    // But don't make it too large for recursive structures like villages or you'll crash
                    // server due to hundreds of pieces attempting to generate!
                    10),
            ...
            // only 1 of that piece exists, etc. But you do not have access to the piece's blocks as this
            // list
            // holds just the piece's size and positions. Blocks will be placed later in JigsawManager.
            //
            // In this case, we do `piece.offset` to raise pieces up by 1 block so that the house is not
            // right on
            // the surface of water or sunken into land a bit.
            //
            // Then we extend the bounding box down by 1 by doing `piece.getBoundingBox().minY` which will
            // cause the
            // land formed around the structure to be lowered and not cover the doorstep. You can raise
            // the bounding
            // box to force the structure to be buried as well. This bounding box stuff with land is only
            // for structures
            // that you added to Structure.NOISE_AFFECTING_FEATURES field handles adding land around the
            // base of structures.
            //
            // By lifting the house up by 1 and lowering the bounding box, the land at bottom of house
            // will now be
