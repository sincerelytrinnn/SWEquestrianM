...
public class CantazariteAnvilContainer extends AbstractRepairContainer {
   ...
   */
  public CantazariteAnvilContainer(
      int id, PlayerInventory playerInventory, IWorldPosCallable worldPosCallable) {
  ...
  protected boolean mayPickup(PlayerEntity p_230303_1_, boolean p_230303_2_) {
    return (p_230303_1_.abilities.instabuild
        || p_230303_1_.experienceLevel >= this.maximumCost.get());
  }

  protected ItemStack onTake(PlayerEntity p_230301_1_, ItemStack p_230301_2_) {
    if (!p_230301_1_.abilities.instabuild
        && !(this.inputSlots.getItem(0).getItem() instanceof SWEMArmorItem)) {
      p_230301_1_.giveExperienceLevels(-this.maximumCost.get());
    }

    net.minecraftforge.common.ForgeHooks.onAnvilRepair(
        p_230301_1_, p_230301_2_, this.inputSlots.getItem(0), this.inputSlots.getItem(1));
    ...
    if (itemstack.isEmpty()) {
      ...
      j =
          j
              ...
              + (itemstack2.isEmpty() ? 0 : itemstack2.getBaseRepairCost());
      this.materialCost = 0;
      boolean flag = false;
      ...
      if (!itemstack2.isEmpty()) {
        ...
        if (itemstack1.isDamageableItem()
          ...
          if (!flag
              && (itemstack1.getItem() != itemstack2.getItem() || !itemstack1.isDamageableItem())) {
            this.resultSlots.setItem(0, ItemStack.EMPTY);
            this.maximumCost.set(0);
            return;
          }
