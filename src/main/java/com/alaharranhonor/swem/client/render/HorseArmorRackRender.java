  ...
  public void render(
    ...
    if (!(armor.getItem() == Items.AIR || armor == ItemStack.EMPTY)) {
      ...
      while (group.hasNext()) {
        ...
        // the order of the bones are stored in a json array which does not guarantee order
        // preservation - Gecko
        ...
        if (bone.getName().equals("cloth")) {
          ...
          // We need to check the name, because as mentioned about, it's not a guaranteed order
          // preservation, so for some armor pieces, the sides are swapped.
