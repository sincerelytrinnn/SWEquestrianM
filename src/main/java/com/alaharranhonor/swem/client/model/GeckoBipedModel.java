...
public class GeckoBipedModel extends CustomAnimatedGeoModel<GeckoRider> {
	...
	public boolean resourceForModelId(AbstractClientPlayerEntity player) {
		this.animationFileLocation = new ResourceLocation(SWEM.MOD_ID, "animations/animated_player.animation.json");
	...
	public void setRotationAngles(PlayerEntity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float partialTick) {
		...
		this.getCustomBone("LeftClavicle").addRotationY(Math.min(armLookAmount + armLookAmountLeft, 1) * -netHeadYaw * ((float)Math.PI / 180F));
		this.getCustomBone("RightClavicle").addRotationY(Math.min(armLookAmount + armLookAmountRight, 1) * -netHeadYaw * ((float)Math.PI / 180F));
		if (flag) {
			...
			this.getCustomBone("LeftClavicle").addRotationX(Math.min(armLookAmount + armLookAmountLeft, 1) * -headPitch * ((float)Math.PI / 180F));
			this.getCustomBone("RightClavicle").addRotationX(Math.min(armLookAmount + armLookAmountRight, 1) * -headPitch * ((float)Math.PI / 180F));
		}
...
//		}
	...
	public static void breathAnim(CustomGeoBone rightArm, CustomGeoBone leftArm, float ageInTicks, float armBreathAmount) {
